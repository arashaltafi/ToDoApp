...
import com.arash.altafi.todoapp.adapters.OnItemDoneStateListener
...
class MainActivity : AppCompatActivity() {
    ...
    private fun init() {
        ...
        val addNewToDoResultActivity = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == Activity.RESULT_OK) {
                ...
                val toDo : ToDo = ToDo(title = title!! , description = description!! , done = false)
...
//                MainScope().launch { toDoViewModel.insert(toDo) }
        ...
        val editToDoResultActivity = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == Activity.RESULT_OK) {
                val data = it.data ?: return@registerForActivityResult
                ...
                if (id == -1) {
                    ...
                    val title = data.getStringExtra(AddToDoActivity.EXTRA_TITLE)
                    val description = data.getStringExtra(AddToDoActivity.EXTRA_DESC)
                    val toDo : ToDo = ToDo(id = id , title = title!! , description = description!! , done = false)
        ...
        adapter.setOnItemClick(object : OnItemClickListener {
            override fun onItemClick(toDo: ToDo) {
                val intent : Intent = Intent(this@MainActivity , AddToDoActivity::class.java)
        ...
        adapter.setOnItemStateClick(object : OnItemDoneStateListener {
